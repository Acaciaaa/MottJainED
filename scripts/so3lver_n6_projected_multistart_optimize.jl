...
iszero(base.V0) || error(
    "V0 must be fixed to zero in the Laughlin-1/3 projected model",
)
...
outer_parameters == [:Uf, :Uf0, :Vf0] || error(
    "outer parameters must be ordered as Uf,Uf0,Vf0",
)
u0_over_uf = Float64(outer_config["u0_over_uf"])
isfinite(u0_over_uf) && u0_over_uf > 0 || error(
    "outer.u0_over_uf must be finite and positive",
)
...
consensus_objective_tolerance = Float64(
    outer_config["consensus_objective_tolerance"],
)
consensus_tolerance_config = outer_config["consensus_parameter_tolerances"]
consensus_parameter_tolerances = Float64[
    consensus_tolerance_config[String(parameter)] for parameter in outer_parameters
]
all(consensus_parameter_tolerances .> 0) || error(
    "multi-start consensus parameter tolerances must be positive",
)
consensus_objective_tolerance >= 0 || error(
    "multi-start consensus objective tolerance must be nonnegative",
)
...
Set(keys(continuation_steps)) == Set((:Uf, :Uf0, :Vf0, :mu)) || error(
    "continuation steps must contain exactly Uf,Uf0,Vf0,mu",
)
...
signature = MottJainED.stable_id(
    "so3lver-n6-laughlin13-multistart-v1", nm, k, tol, ncv, seed,
    String(heavy_space_mode), u0_over_uf,
...
problem = build_cft_problem(
    nm, base; heavy_space_mode, disp_std=true,
)
...
if isfile(outer_path)
    ...
    for row in eachrow(previous)
        ...
        values = Float64[row.Uf, row.Uf0, row.Vf0]
...
function record_outer!(summary, source, seconds)
    MottJainED.append_csv(outer_path, (
        ...
        seconds=seconds,
        Uf=summary.values[1],
        U0=u0_over_uf * summary.values[1],
        Uf0=summary.values[2],
        Vf=base.Vf,
        Vf0=summary.values[3],
        V0=0.0,
        t=base.t,
    ))
    return nothing
...
function profile_mu(values; source="outer", force=false)
    ...
    fixed = MottJainED.with_coupling(
        fixed, :U0, u0_over_uf * fixed.Uf,
    )
    warm = anchor_warm()
...
# A broad search is only scientifically conclusive when the independent local
# runs in the final trust region return to the same basin.  Reprofile every
# reported minimizer so consensus is based on complete inner-mu searches, not
# only Optim's cached scalar value.
final_round = maximum(Int(run["round"]) for run in local_run_rows)
consensus_rows = NamedTuple[]
for run in local_run_rows
    round_index = Int(run["round"])
    start_index = Int(run["start_index"])
    considered = round_index == final_round
    minimizer = Float64.(run["minimizer"])
    summary = profile_mu(
        minimizer;
        source="consensus_round_$(round_index)_start_$(start_index)",
    )
    parameter_deltas = abs.(summary.values .- best.values)
    agrees = considered && summary.valid &&
             all(parameter_deltas .<= consensus_parameter_tolerances) &&
             summary.objective <= best.objective + consensus_objective_tolerance
    push!(consensus_rows, (
        round=round_index,
        start_index=start_index,
        ...
        objective=summary.objective,
        delta_objective=summary.objective - best.objective,
        Uf=summary.values[1],
        Uf0=summary.values[2],
        Vf0=summary.values[3],
...
multistart_consensus_fraction = count(row -> row.agrees_with_best, final_consensus_rows) /
                                length(final_consensus_rows)
multistart_all_agree = all(row -> row.agrees_with_best, final_consensus_rows)
...
for (rank, candidate) in enumerate(first(
    ...
    push!(top_rows, (
        ...
        Uf=candidate.values[1], U0=u0_over_uf * candidate.values[1],
        Uf0=candidate.values[2], Vf0=candidate.values[3],
        V0=0.0, mu=candidate.best_mu,
    ))
end
...
best_dict = Dict{String,Any}(
    ...
    "multistart_final_start_count" => length(final_consensus_rows),
    "multistart_consensus_fraction" => multistart_consensus_fraction,
    "multistart_final_round" => final_round,
    ...
    "parameters" => Dict(
        "Uf" => best.values[1], "Uf0" => best.values[2],
        "U0" => u0_over_uf * best.values[1],
        "Vf" => base.Vf, "Vf0" => best.values[3], "V0" => 0.0,
...
println("multistart_all_agree=$multistart_all_agree " *
        "multistart_complete=$multistart_complete " *
        "consensus_fraction=$multistart_consensus_fraction")
println("best_objective=$(best.objective) q6=$(best.q6) q5=$(best.q5)")
println("best_parameters=Uf=$(best.values[1]),U0=$(u0_over_uf * best.values[1])," *
        "Uf0=$(best.values[2]),Vf0=$(best.values[3]),V0=0.0," *
        "mu=$(best.best_mu)")
